<?php
// Configuration File for Hosting (PostgreSQL)
// Replace these values with your live server database credentials

define('DB_HOST', 'dpg-d8hc255dt1ts7389ic90-a.ohio-postgres.render.com'); 
define('DB_PORT', '5432');      
define('DB_USER', 'blog_user');  
define('DB_PASS', 'uCITM3bW7mOlVyZQLlwLhCF52qSF9WGs');          
define('DB_NAME', 'blog_db_wcgu');   

// Optional: Base URL of your website (e.g., https://yourdomain.com/)
define('https://xyz-blogs.onrender.com/', 'http://localhost/XYZ_Blogs/');
?>
